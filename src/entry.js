...
async function pipelineStatus(db) {
  return await db.prepare("SELECT id,run_date,status,stage,started_at,finished_at,collection_run_id,workflow_id,candidate_count,selected_count,editorial_id,error FROM daily_pipeline_runs ORDER BY id DESC LIMIT 1").first();
}
...
function looksLikeDump(body) {
  ...
  if (text.indexOf("原文摘要") >= 0) return true;
...
async function readWorkflow(env, workflowId) {
  ...
  try { return await (await env.DAILY_DISCOVERY.get(workflowId)).status(); } catch (_) { return null; }
}
...
async function recoverIfNeeded(env) {
  ...
  if (dump) {
    try {
      ...
      return { recovered_stale_run: true, new_workflow_id: null, packed: packed };
    } catch (_) {}
  }
  if (run && run.status === "success" && !dump) return { recovered_stale_run: false, new_workflow_id: null };
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === "/episode" || url.pathname.startsWith("/episode/")) return episodeResponse(env.DB, url.pathname.split("/")[2]);
    if (url.pathname.startsWith("/audio/")) return audioResponse();
    if (url.pathname === "/rebuild") {
      await ensureExtendedTables(env.DB);
      const packed = await packThree(env, null);
      return Response.json({ ok: true, version: VERSION, packed: packed });
    }
    if (url.pathname === "/run") {
    ...
    if (url.pathname === "/status") {
      ...
      return Response.json({ ok: true, version: VERSION, pipeline: pipeline, workflow: workflow });
    }
    if (url.pathname === "/") {
      return Response.json({ name: "Global Discovery Engine", status: "online", version: VERSION, rss: "/podcast.xml", pipeline: await pipelineStatus(env.DB) });
    }
